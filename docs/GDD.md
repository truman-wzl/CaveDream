...
**梦之主（角色与捏脸）**
- **捏脸 = 像素画板上色**（见 §3.10）：引擎给带部位划分的白色底模，玩家从全局调色板逐格涂色自由造型；**不再是性别/发型/肤色的枚举组合**。目标仍是被记住的经典形象（默认底模即海报形象）。
...
**数据模型**：`PaintedLook{ templateId, int[] colors }` + 底模 `regions()`（部位划分：头发/皮肤/眼睛/腮红/衣袍/手臂/腿/鞋）+ `Palette`（**黑白 + RGB 彩虹**：由 HSL 色相环×多档明度 + 灰阶程序生成，可扩容；上色一律吸附最近项、全局美术统一）。梦之主=模板 0；NPC/方块/武器各占模板 id。

**已落地（v0.49）**：
- 全局调色板 `Palette`（**黑白灰阶 + HSL 色相环彩虹**，程序生成、自动吸附）；
- **捏脸画板 `PaintStudioScreen`**：选职业后、世界生成前进入；画笔/橡皮/填块(整片部位)/重置 + 实时预览 + `Enter 完成·入梦`；玩家在**整幅画布**自由上色（底模仅起始模板，体外亦可涂→可自由造型）；
...
- **构梦者 `GuideNpc`**（漫游商人，非敌对）：玩家**铸梦币≥10** 自动在村庄出现；走近**右键**开商店。
- **商店**：花 **50 铸梦币**一次性购买 **《世界准则法典》**（`GameSave.codexOwned` 权限标记，非背包物品）。
- **按 X 开法典菜单 → 选画板 → 进画板重绘 → 回世界即时生效**（`PaintStudioScreen` 重绘模式 + `CaveDreamGame` 屏返回 + `PlayScreen.applyRepaint`）。样貌画板已通；方块/武器画板占位“开发中”。
...
- **墙类规范**：背景墙（`WOOD_WALL`）**非实体、可穿行、堆叠上限 999**（1 木→4 木墙）；铺满房屋内部即满足合法房屋“贴满墙”、且室内不刷怪。墙上可挂**非实体装饰**（火把/吊灯/画/NPC 头像）——需**墙面对象第二层**（与背景墙同格共存），为后续待办。
...
| P2 | ~~房屋判定~~ ✅ 已定稿（v0.23.1 → §3.7 · v0.50 尺寸上调 12×8） | 内部≥12×8/贴满墙/桌椅每客一套/光源/≥1 扇门且全密闭；容量=min(面积÷24, 套件数) | — |
