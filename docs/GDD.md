...
- 登录/账号界面（v0.50）：标题屏“账号”→**木板风格**面板（WoodUi 配色、按钮悬停浮亮 + 微浮动），三模式 登录/注册/找回密码 + **用户中心**（登录态：显示当前昵称、改昵称、登出）；登录字段“账号(用户名/邮箱)”；CloudAuthClient 异步不卡渲染；会话存 `~/cavedream/session.json`（token+昵称；后端重启则 token 失效需重登）。
