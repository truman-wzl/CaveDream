...
> 版本 **v0.39** · 主题：梦境 · 类型：2D 沙盒冒险 + RPG（类泰拉瑞亚）· 中文名：**梦迹行者**
...
- 耗材/材料 300+（待用）。客户端 `Item` 与后端 `t_item_def` 共用此 id。
...
| P7 | 动态视差背景 | 多层背景随相机视差滚动 + 循环动画：MC 式飘云、泰拉瑞亚式各生物群系大图（天空带/地表/洞穴/地狱/海 不同背景）；含太阳/月亮升落 | M3 |
