...
> 版本 **v0.53**（里程碑 M2 进行中）· 主题：梦境 · 类型：2D 沙盒冒险 + RPG（类泰拉瑞亚）· 中文名：**梦迹行者**
...
**纹理路线（v0.50）**：以**程序化 16px 图集**为主（噪点/autotiling，零素材依赖）+ **玩家/NPC 画板上色**（§3.10）；后期可选 Aseprite 自绘替换。~~旧“M2-b Kenney/CC0 包替换”路线已弃~~。
...
**梦之主（角色与捏脸）**
- **捏脸 = 像素画板上色**（见 §3.10）：引擎给带部位划分的白色底模，玩家从全局调色板逐格涂色自由造型；**不再是性别/发型/肤色的枚举组合**。目标仍是被记住的经典形象（默认底模即海报形象）。**默认底模（梦之主）v0.51 重绘**：由旧“高额发际线+秃头”改为**齐刘海盖额、偏大眼、头顶呆毛、背后一束飘溢长发**的年轻形象。
...
### 3.10 造梦画板（灵魂画师）与世界准则法典（v0.49 · v0.50 扩展）
...
- 当前：**程序化像素为主**（`BlockTextures` 噪声/autotiling + `Palette` 全局调色板），**零外部素材依赖**；玩家/NPC 外观由**画板上色**（§3.10）产出，作者与玩家共创。
...
| P11 | 存档版本迁移 | `GAME_VERSION` 已作判据但无实际迁移逻辑；字段增改（如 `paintTemplate`→`paintW/H`）致旧档兼容需系统化读档兜底/迁移（v0.51 已加越界防崩，仍待正式版本迁移） | M3 |
| P12 | 墙面装饰与家具扩展 | 墙面对象第二层已落地（火把可挂墙、v0.51）；待扩：吊灯/画/NPC 头像挂墙、熔炉站位校验与冶炼界面、锤/铲工具与蜡烛 | M3 |

---

## 附录 A · 实现状态总览（截至 v0.53）

> **一句话**：单机可玩的“**沙盒核心循环**”已跑通并有单测/打包验证——世界生成 / 移动跳跃游泳 / 挖掘建造 / 战斗掉落 / 合成熔炼 / 存储 / 本地+云存档 / 账号登录 / 画板外观 / NPC 法典；**RPG 进度层（多层世界·Boss·职业技能·装备护甲饰品·品质附魔·锻造）与世界事件层（魇潮·资源重生·梦锚柱）尚未开发**。图例：✅ 已落地　🟡 框架在/内容占位　⬜ 设计已定、代码未动。
...
| 法典 / 画板 | 《世界准则法典》X 随处开、**两级列表选择器**（NPC/物品，注册式）、物品重绘取自身贴图、`PaintStudioScreen` 捏脸（部位大分区快填）、默认底模重绘 | §3.10 |
