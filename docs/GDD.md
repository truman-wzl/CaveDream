...
> 版本 **v0.50** · 主题：梦境 · 类型：2D 沙盒冒险 + RPG（类泰拉瑞亚）· 中文名：**梦迹行者**
...
- **天空岛群扩容（v0.52）**：浮岛群体量×3（`WorldGenerator` 浮岛半径×~1.73≈面积×3，但锁不超图宽/5 以免横贯雾墙隔断连通），且**岛体新增 `STONE` 石核**→可挖掘；对天空带跑 `oreVeins(host=STONE)` → **岛上随机分布铁矿/金矿**；草面变大使 `plantIslandTrees` 植树更明显。中央主岛（出生/入梦锤所在）不随浮岛群放大。
...
- **击杀掉币 / 拾取 / 自动转身（v0.49–v0.50）**：怪死亡**掉落银色铸梦币实体**（`Item.COIN` id 400，受重力、可磁吸、拾取才入账、不占背包格）；掉币数**由怪物自身 `Mob.rollCoins()` 决定**（史莱姆 2~4、蝙蝠 3~6，新怪各设各的），**集中在 `SpawnManager.drainKilled()` 结算**→近战/弹道/仆从等任何击杀来源都掉；玩家**物品拾取磁吸半径初始 3.5 格**；**挥动武器时按鼠标与角色左右相对位置自动转身**（起手与武器运动期间持续跟随）；**鼠标滚轮循环切换快捷栏**（换工具/武器）。
...
- 方块 `0–26`（沿用 BlockType.id；铁矿13/金矿14/白金矿15/圣矿16/**木门17**/**木平台18/桌子19/椅子20/工作台21/床22/熔炉23/木箱24/火把25/木墙26**）；
...
- **多格占地** `footprintW/H`：木箱 **2×2**、桌子/工作台 **2×1**；`placeBlock` 整体放置（需全格空、不压玩家）、`breakBlock` 整体破坏掉 1、`anchorOf` 从任一格反推左下角锚点。
...
- **熔炉与冶炼**：`FURNACE` 工作站；配方把 铁矿/金矿/白金矿/圣矿 → **金属锭**（id 300–303，Kind.MATERIAL）。“须在熔炉旁才能冶炼”的站位校验待接。
...
| P8 | 植被物品化 | 树/花草/蕨 地表植被作为可破坏→掉落→拾取入包（树木方块已可挖落，花草待） | M3 |
