...
> 版本 **v0.49** · 主题：梦境 · 类型：2D 沙盒冒险 + RPG（类泰拉瑞亚）· 中文名：**梦迹行者**
...
- **击杀掉币 / 拾取 / 自动转身（v0.49）**：史莱姆等怪死亡**掉落银色铸梦币实体**（`Item.COIN` id 400，受重力、可磁吸、拾取才入账、不占背包格）；玩家**物品拾取磁吸半径初始 3.5 格**；**挥动武器时按鼠标与角色左右相对位置自动转身**（起手与武器运动期间持续跟随）。
...
- **背包面板（v0.48 落地）**：按 **E** 弹木质公告牌面板，**左侧竖排三标签 = 背包 / 合成 / 装备与饰品**（默认停在背包）。背包页 40 格（含快捷栏前 10）、点格选中、**点武器即自动装备**；装备页 = 武器1+护甲4+饰品3（点已填槽卸回背包，背包满则放回）；当前仅武器可装（护甲/饰品待物品体系接入自动生效）。
...
- 耗材/材料 `300–999`（待用）；
- **敌怪 `1000–1999`**：史莱姆 = **1000**（`Slime.ID`，`Mob.typeId()`），后续怪顺延；仆从/NPC/Boss 各占段（预留 2000+/3000+…）；
- **配方 `5000–5999`（v0.48）**：`Recipe.id`，如 5001 石镐、5002 石斧、5003 铁镐、5004 铁斧（输入=物品 id+数量，输出=物品，未解锁不显示）；
- **外观模板 id（v0.49）**：`PaintedLook.templateId`（0=梦之主；NPC/方块/武器各占段），成品为每格 ARGB 的上色数据，用全局调色板（**黑白灰阶 + 彩虹**、程序生成）；
...
**规划（法典 = 世界编辑/UGC 入口）**：
- **构梦者**（NPC）以**铸梦币**出售 **《世界准则法典》**，解锁三类“改画板”：
  1. **改梦境 NPC 样貌画板**（复用 `PaintedLook`，按 NPC 模板 id）；
  2. **改梦境方块画板**（重涂 `BlockType` 贴图，写回 `t_item_form` 图片形态）；
  3. **改梦境武器画板**——**画板样式随职业而变**（战士剑/法师杖/射手弓…各有底模）；
- 特定**剧情节点触发**画板小游戏；作者侧导出通道（`AssetBaker`→DB）与玩家侧同编辑器；与 §3.5 存档、数据驱动渲染、`t_item_form` 图片入库天然打通。
